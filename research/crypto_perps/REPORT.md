...
Deploy-gate **PASS**. Proceed to build **at the Amendment B profile** (Amendment A knobs + hysteresis-hold + no dd-tiers + band-edge rebalancing + cash-yield layer; operator-selected 2026-07-08 over the 3× alternative). **Final production numbers** (`production` scenario, permanent in the suite; 3.5% yield via Coinbase One Basic minus subscription): **+40.7% CAGR, Sharpe 1.14, max DD 58.6%, 2023+ Sharpe +0.73; 2×-cost stress +33.2%/0.98; no halt.** Riders: (1) expectations per Amendment A — ~30% CAGR central case with ~50% drawdowns and red years (2021/2025-shaped) as a normal part of the deal; (2) BTC-only at launch per F-2; (3) funding telemetry from day one and §7-over-§6 precedence in the sizing engine per F-3/F-4; (4) the $1,500 halt ships as a malfunction circuit-breaker — removing it entirely was evaluated and adds nothing (identical backtest) while removing the debit/runaway backstop.
