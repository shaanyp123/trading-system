...
>
> **Final validated numbers (2017→2026H1, `production` scenario in `research/crypto_perps/results.json`; cash yield 3.5% via Coinbase One Basic minus $4.99/mo subscription — USDC rewards are subscriber-gated since 2025-12): +40.7% CAGR, Sharpe 1.14, max DD 58.6%, 2023+ Sharpe +0.73; 2×-trading-cost stress +33.2% / Sharpe 0.98; worst year −4% (2024); avg gross leverage 0.50x, peak 1.72x; no halt/bust.** The 3× alternative (+~48% CAGR, Sharpe ~1.05, DD ~66%) was evaluated and REJECTED: lower Sharpe, and only ~9 pts of drawdown headroom against the operator's 75% budget vs ~16 pts here. Morpho USDC lending (incl. the ~4.77% high-yield vault) REJECTED for system cash — vault liquidity queues correlate with exactly the stress events in which the margin account needs same-day cash (decisions-log 2026-07-08). Scale-up beyond this profile is a **post-live decision** (≥6 months of live data), not a launch parameter.

> **⚠️ AMENDMENT C (2026-07-18, operator-directed) — hard-halt floor measurement basis = TOTAL capital.** Context: the 2026-07-17 `DECOMMISSION_FLOOR` halt (decisions-log 2026-07-18 chain) showed the $1,500 malfunction floor — Amendment A's "25% of initial $6,000" circuit-breaker — armed at normal operating equity under the Amendment B cash-split posture, because its input (`equity_from_summary`, the CFM+CBI **USD** complex) cannot see the CBI-USDC cash-yield balance; a routine ~$52 annual subscription charge tripped it. Operator decision (queued-decision option **b**): the floor is measured against **total capital = venue USD equity + the latest `cash_balance_snapshots` CBI-USDC capture**, with **fail-safe bias**: if the USDC snapshot is missing, malformed, or older than 48 h, the check falls back to USD-only — since USDC ≥ 0 can only raise the basis, every fallback is a tightening, never a loosening. The floor **value stays $1,500**; breach behavior (flatten + persistent halt, manual clear) is unchanged. **Strictly out of scope:** sizing equity (the sizer keeps reading venue USD only — LOCKED), daily/weekly loss limits, and all signal/engine parameters.
