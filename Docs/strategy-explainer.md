...
| 2026-07-20 | Binance funding proxy SIDELINED (default OFF): Binance geo-blocks US IPs (HTTP 451 from the VPS), so the B3 comparison series cannot capture from this host. Gate B3 stays `insufficient_data` until an alternate proxy source that serves US IPs exists — a named open item for the C2 graduation decision. | First live capture attempt failed at deploy; operator decision to sideline rather than route around the block; decisions-log 2026-07-20 addendum. |
