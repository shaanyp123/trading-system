...
- [ ] **Phase 1 cutover (architectural fix for webhook_pusher delivery)** — deploy `webhook_pusher` as its own service container per the existing docker-compose.yml stub (`networks: [internal, egress]; profiles: ['phase1']`). Requires: (a) new `services/webhook_pusher/Dockerfile` mirroring the `services/api/Dockerfile` shape (multi-stage build, hardened runtime, non-root user); (b) Week 4 risk dispatcher invokes `dispatch_alert(...)` either as a sidecar process or via a queue mechanism (Phase 1 spec hasn't pinned the IPC pattern yet); (c) `docker-compose.yml` removes the `phase1` profile gate from `webhook_pusher` so it starts on every Phase 1+ bringup; (d) `deploy/webhook_pusher/README.md` Step 0 + Step 8 network-attach workaround removed since `webhook_pusher` (not `api`) is the smoke target. Until then, Phase 0 keeps the network-attach workaround. See "Day 11 carryover #2" entry above for the architectural rationale (option A vs B vs C).
- [ ] **CI gate to prevent Dockerfile/pyproject.toml dep drift** (queued as a separate small PR after PR #48). Tiny Python script that parses `pyproject.toml` `[project] dependencies` + the api Dockerfile's hardcoded pip-install list, fails CI if they differ. Add as a `dep-drift-check` job in `.github/workflows/ci.yml` (or fold into `lint`). Would have caught PR #44's missing httpx + resend at PR #44's own CI run instead of requiring me to debug it Day 11.
