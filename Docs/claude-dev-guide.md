...
**Phase 1 architecture (CRITICAL — REVISED 2026-05-12 per DP-025 → Option 4; data-layer sub-pivot 2026-05-20):**
- **Backend HAS direct IBKR connection** via `ib-async` to a Dockerized `ib_gateway` container on **`clientId=1`** (locked since PR #101 / `services/execution/ibkr_adapter.py::DEFAULT_CLIENT_ID = 1`).
- **LEAN runs locally on the operator's VPS** in a custom-built `lean_local` container (extends `quantconnect/lean:latest` with `QuantConnect.Brokerages.InteractiveBrokers` plugin DLL baked in via NuGet at image build time — `infrastructure/lean_local/Dockerfile` multi-stage); LEAN POSTs signal events to backend at `POST /api/internal/lean/signals` (shared-bearer auth, `LeanAuthMiddleware` analogous to Day 23's `BotAuthMiddleware`).
- **LEAN reads market data from IBKR on `clientId=10`** via the `InteractiveBrokersBrokerage` data-queue-handler + history-provider (delayed quotes + `reqHistoricalData`; `ib-enable-delayed-streaming-data=true`). 2026-05-19 evening probe confirmed delayed-quote actual wall-clock lag is 1-3 seconds (not the 10-15 min IBKR advertises) and reqHistoricalData returns current-trading-day bars including the settlement bar — both FREE for any IBKR account holder. LEAN's `live-mode-brokerage` stays `PaperBrokerage`; LEAN does NOT place orders.
- **No seed files; no DataBento; no yfinance.** Pre-2026-05-20 architecture read on-disk seed files via `FakeDataQueue` + `SubscriptionDataReaderHistoryProvider`; that path is retired. The `trading_lean_data` Docker volume mount stays for rollback safety; operator can `docker volume rm trading_lean_data` after a 30-day soak.
- **IBKR clientId allocation (LOCKED 2026-05-20):** **api order-placement worker = 1**, **LEAN data-queue-handler = 10**, **operator probes + recovery tools = 80-99** (e.g., `scripts/operator_tools/replay_executions.py` uses clientId=99). Reserve 2-7 for future expansion (multi-strategy, read-only telemetry clients). Document the clientId in every new IBKR-connecting code path. IBKR allows multiple distinct client-ids per gateway session (verified 2026-05-19 probe) but a clientId collision raises Error 162 "Trading TWS session is connected from a different IP address" + wedges the colliding client for ~30 min. Never overlap.
...
- See `Docs/decisions-log.md` 2026-05-12 entry "Phase-1 architecture pivot — QC ObjectStore → LEAN Local + direct IBKR" + 2026-05-20 entry "Phase 1 data-layer pivot: IBKR delayed quotes replace seed-file architecture" for the full rationale + diff lists.
