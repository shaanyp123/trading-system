...
- **LEAN runs locally on the operator's VPS** in a custom-built `lean_local` container (extends `quantconnect/lean:latest` with tini + pyyaml + structlog only — no IBKR plugin DLLs under Option C); LEAN POSTs signal events to backend at `POST /api/internal/lean/signals` (shared-bearer auth, `LeanAuthMiddleware` analogous to Day 23's `BotAuthMiddleware`).
- **LEAN reads market data from on-disk** via the original `FakeDataQueue` + `SubscriptionDataReaderHistoryProvider` shape (read-only mount of the shared `lean_data` Docker volume). LEAN does NOT connect to IBKR under Option C — the api's bar_sync worker is the sole producer; lean_local is a pure consumer. `live-mode-brokerage` stays `PaperBrokerage`; LEAN does NOT place orders. The 2026-05-20 v1 attempt's `InteractiveBrokersBrokerage` data-queue-handler on clientId=10 is RETIRED (blocked at deploy by the IBAutomater gateway-launch conflict; see `Docs/decisions-log.md` 2026-05-20 evening entry).
- **No DataBento; no yfinance.** The pre-2026-05-20 seed-file architecture (yfinance ETFs + DataBento futures) is retired. The `lean_data` Docker volume is now actively written by the api each day. Operator can `docker volume rm trading_lean_data` only after retiring Option C — but Option C IS the canonical architecture, so the volume is load-bearing.
- **IBKR clientId allocation (LOCKED 2026-05-21; bar_sync synced to deploy reality in 2026-05-21 follow-up):**
  - **api order-placement worker = 1** (long-lived; orders + position queries + EOD recon). Deploy override at VPS `deploy/.env` sets `API_IBKR_CLIENT_ID=2`; the code default + this guide remain at `1` pending an operator decision to reconcile.
  - **api bar_sync worker = 3** (per-cycle connect/disconnect; reqHistoricalData for the 11-market universe). `clientId=2` reserved for the order-worker's deploy override so neither side collides.
  - **EOD recon positions client = 4** (per-cycle connect/disconnect; `reqPositions` view for the EOD position-quantity check, per `services/reconciliation/ibkr_intraday.py::DEFAULT_RECON_CLIENT_ID`). Added 2026-05-28 under Option C; the module is dead code until a follow-up PR wires it into `run_eod_cycle`, so recon position-qty stays on FlexQuery via clientId=1 until then.
  - **operator probes + recovery tools = 80-99** (e.g., `scripts/operator_tools/replay_executions.py` uses clientId=99)
  - **reserve 5-7** for future multi-strategy / additional read-only telemetry clients
  - Document the clientId in every new IBKR-connecting code path. IBKR allows multiple distinct client-ids per gateway session (verified 2026-05-19 probe + 2026-05-21 with Option C's order-worker + bar_sync running concurrently in production) but a clientId collision raises Error 162 "Trading TWS session is connected from a different IP address" + wedges the colliding client for ~30 min. Never overlap.
- **LEAN data source = on-disk via api-managed bars.** The api's `BarSyncWorker` fetches via `reqHistoricalData` on clientId=3 + writes the LEAN on-disk format (equity-daily zip with deci-cent integer-scaled prices + futures-daily zip with raw float prices + per-day universe CSVs + map_files sentinels). LEAN reads via `FakeDataQueue` + `SubscriptionDataReaderHistoryProvider` on the read-only volume mount. Single-writer-multiple-reader on the shared `lean_data` Docker volume.
- Order placement, cancellation, position queries, and EOD reconciliation are all direct against IBKR on clientId=1; **no instruction protocol; no QC ObjectStore polling**.
