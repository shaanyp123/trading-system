...
**Phase 1 architecture (CRITICAL — REVISED 2026-05-12 per DP-025 → Option 4; data-layer pivot v2 2026-05-21 Option C):**
- **Backend HAS direct IBKR connection** via `ib-async` to a Dockerized `ib_gateway` container on TWO distinct clientIds:
  - **`clientId=1`** — long-lived order-placement worker (`services/execution/ibkr_adapter.py::DEFAULT_CLIENT_ID = 1`; locked since PR #101).
  - **`clientId=2`** — per-cycle bar-sync worker (`services/data/bar_sync.py::DEFAULT_BAR_SYNC_CLIENT_ID = 2`; locked 2026-05-21 with Option C). The worker fires once per ET calendar day at 17:00 ET, opens a fresh ib-async connection, fetches `reqHistoricalData` for all 11 Phase 1 markets, writes to the shared `lean_data` Docker volume in LEAN's on-disk format, disconnects.
...
- **IBKR clientId allocation (LOCKED 2026-05-21):**
  - **api order-placement worker = 1** (long-lived; orders + position queries + EOD recon)
  - **api bar_sync worker = 2** (per-cycle connect/disconnect; reqHistoricalData for the 11-market universe)
  ...
  - Document the clientId in every new IBKR-connecting code path. IBKR allows multiple distinct client-ids per gateway session (verified 2026-05-19 probe + 2026-05-21 with Option C's clientId=1 + 2 running concurrently in production) but a clientId collision raises Error 162 "Trading TWS session is connected from a different IP address" + wedges the colliding client for ~30 min. Never overlap.
- **LEAN data source = on-disk via api-managed bars.** The api's `BarSyncWorker` fetches via `reqHistoricalData` on clientId=2 + writes the LEAN on-disk format (equity-daily zip with deci-cent integer-scaled prices + futures-daily zip with raw float prices + per-day universe CSVs + map_files sentinels). LEAN reads via `FakeDataQueue` + `SubscriptionDataReaderHistoryProvider` on the read-only volume mount. Single-writer-multiple-reader on the shared `lean_data` Docker volume.
