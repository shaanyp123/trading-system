...
def test_main_maps_coinbase_credentials(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def fake_exec(cmd: str, args: list[str]) -> None:
        captured["key_name"] = os.environ.get("API_COINBASE_API_KEY_NAME")
        captured["private_key"] = os.environ.get("API_COINBASE_API_PRIVATE_KEY")

    monkeypatch.setattr(os, "execvp", fake_exec)
    entrypoint.main(argv=["true"])

    assert captured["key_name"] == "organizations/abc/apiKeys/def"
    assert captured["private_key"] is not None
    assert "EC PRIVATE KEY" in captured["private_key"]


def test_main_skips_placeholder_coinbase_credentials(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Placeholder coinbase keys are NOT exported (fail-closed: the
    strategy worker refuses to start without real credentials)."""
    secrets_path = _write_yaml(
        tmp_path,
        """
        postgres:
          app_service_password: realhex123
        coinbase:
          api_key_name: <TODO_FROM_C0_COINBASE_KEY_CEREMONY>
          api_private_key: <TODO_FROM_C0_COINBASE_KEY_CEREMONY>
        """,
    )
    monkeypatch.setenv("API_SECRETS_PATH", str(secrets_path))
    monkeypatch.setenv("API_ENVIRONMENT", "paper")
    ...
    assert captured["key_name"] is None
