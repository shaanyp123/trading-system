...
### Code + ops surfaces (current state as of 2026-05-10, Day 12 carryover)
| Path | Purpose | Status |
|---|---|---|
...
| `tests/integration/test_audit_writer.py` | testcontainers Postgres 16; 6 tests: single insert chain math, genesis prev_hash = zero32, 3 concurrent writers (continuous chain), deterministic SQLSTATE 40001 retry-path (4 from Day 8); `test_verify_chain_count_matches_select_count` + `test_cli_verify_function_runs_against_real_db` (2 from Day 12 — locks SQL contract for `<N>` in operator runbook + exercises CLI's `_verify` engine-lifecycle wrapper end-to-end) | Day 8 PR #39 (initial); Day 12 PR #51 (verify_chain SQL contract + CLI engine-lifecycle integration) |
...
| `services/audit/` | `event_types.py` (locked taxonomy enum mirror of backend-spec §3.30) + `models.py` (`AuditLogRecord` DTO) + `chain.py` (in-tree JCS + SHA-256 record-hash + `verify_chain` async walker — return type extended Day 12 from `tuple[bool, int \| None]` to `tuple[bool, int \| None, int]` with rows-walked count) + `writer.py` (`append_audit_event` — `pg_advisory_xact_lock` + SERIALIZABLE + 5-attempt SQLSTATE-40001 retry) + `decision_diary.py` validator (PR #28) + `verify_chain.py` operator CLI (Day 12 PR #51 — argparse + asyncio.run + `DATABASE_URL` env-var shell over the chain walker; output contract: `CHAIN OK: <N> rows verified` exit 0 / `CHAIN BREAK at sequence_no=<X> (after <K> verified rows)` exit 1 / usage-error exit 2; required `--env` flag mirrors `audit_log.env` CHECK constraint) | Day 8 PR #39 (canonical hash-chain entry point); Day 12 PR #51 (operator-facing chain verifier; A02 binding; A27 satisfier in `deploy/audit/README.md`); A01 enforceable from PR #39 forward |
