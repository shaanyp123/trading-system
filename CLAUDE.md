...
> **🛑 CME paper trading is RETIRED (crypto-pivot Phase C0, 2026-07-08).** The IBKR/LEAN/CME stack was decommissioned at C0 start per `Docs/crypto-pivot-delta-spec.md` §1 (repo-side: the C0-D1 decommission PR; VPS-side: `deploy/cme-paper-decommission.md`). **No strategy is currently trading.** The Coinbase CFM crypto-perps system (nano BTC/ETH, Amendment B profile) is being built per delta spec §5: C0 (build + offline gates) → C1 (small-live, no shadow phase — operator directive) → C2 (full size).

> **🔄 Architecture (C0 build window).** Chassis retained: FastAPI api, hash-chained audit log, SSE, kill-switch FSM, Discord bot + webhook pusher, recon diff engine, auth, frontend shell. Being replaced per delta spec §3: broker adapter (`coinbase_client`/`coinbase_adapter`), market data + funding telemetry (`coinbase_market_data`), in-process signal engine (`services/signal/crypto_trend.py`, 00:05 UTC daily decision + 30 s risk loop), recon fetcher, cash-yield worker. `services/execution/ibkr_client.py`+`ibkr_adapter.py` and `services/reconciliation/{flex_query_fetcher,ibkr_intraday}.py` remain in-tree until their Coinbase replacements land (Replace disposition, §3.1/§3.5); `strategies/v1_trend_following/` remains until the §3.4 sizing replacement PR unwires `services/risk/sizing.py` + `services/api/repos/exit_proximity.py` from it (risk-review-approved).
