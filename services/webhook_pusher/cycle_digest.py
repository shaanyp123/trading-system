"""services/webhook_pusher/cycle_digest.py — §3.8 daily-decision digest (pure).

Crypto-pivot delta spec §3.8: the ``/cycle`` daily-decision outcome
digest (per-asset trend score → target → action → est. costs) rendered
from the ``GET /api/system/cycle`` payload (§3.7). ANNOUNCE-ONLY
(dev-guide §1.5 locked): the embed carries no buttons, no approval
affordances.

This module is PURE (stdlib + ``event_pushes`` dataclasses only — no
httpx, no sqlalchemy, no I/O) because it is SHARED across two runtimes:

* the webhook_pusher's 00:10 UTC scheduled push
  (:mod:`services.webhook_pusher.cycle_digest_scheduler`) → #daily-brief;
* the Discord bot's on-demand ``/cycle`` slash command
  (``services/discord_bot/commands/cycle.py`` renders the same dict via
  ``discord.Embed.from_dict``). The bot image ships neither sqlalchemy
  nor the pusher's dispatcher — keeping this module import-light is what
  makes the shared-renderer design possible.
...
:func:`plan_cycle_digest_push` additionally implements the scheduled-push
skip: ``None`` when no decision has EVER been recorded (fresh DB / worker
...
whitelist. [A05]: all money values pass through as the api's
Decimal-as-string payloads — no float math here. [A06]: every datetime
tz-aware UTC.
"""
