...
- [ ] `python3 services/audit/verify_chain.py --env paper` returns `CHAIN OK: N rows verified`
      ...
      **Status (2026-05-13, Day 12):** code SHIPPED — `services/audit/verify_chain.py` CLI shipped via PR #51 with `risk-review-approved` label (Day 12 / Week 4 Tue). Argparse + asyncio.run + `DATABASE_URL` env-var shell over the canonical `services.audit.chain.verify_chain` async walker (Day 8 PR #39). Walker's return signature extended to `tuple[bool, int | None, int]` so the CLI's `CHAIN OK: N rows verified` message sources `N` from the walker itself (single source of truth; no race vs concurrent writers from a separate `SELECT COUNT(*)`). 21 new tests across 3 surfaces (5 walker via fake-session + 16 CLI via monkeypatched `_verify` + 2 testcontainers integration); 433/433 tests green. A27 satisfied via `deploy/audit/README.md` 5-step operator runbook (same shape as `deploy/api/README.md` Day 5 + `watchdog/README.md` Day 4 + `deploy/webhook_pusher/README.md` Day 10/11). **Gate flips to `[x]` once operator runs `deploy/audit/README.md` Step 3 on Ashburn** (after the prerequisite VPS git pull + `docker compose build api && up -d --force-recreate api` to bake the new module into the deployed image). Expected output for Phase 0 fresh paper: `CHAIN OK: 0 rows verified` exit 0 (vacuously intact — no Phase-0 writers run yet; first audit events land Week 4 Wed dispatcher work). See `Docs/decisions-log.md` "Day 12 09:00 — services/audit/verify_chain.py CLI + deploy/audit/README.md operator runbook (PR #51)" entry.
